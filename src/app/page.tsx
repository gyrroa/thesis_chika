    ...
    <main className="flex justify-center min-h-dvh bg-[url('/background.svg')] bg-cover bg-no-repeat sm:pt-0 py-[30px]">
      <div className='flex flex-col items-center text-center gap-[20px] sm:gap-[41px] select-none m-0 sm:m-auto'>
        ...
        <div className="flex flex-col gap-[20px] w-[311px] sm:w-[330px]">
          <Button variant="default" onClick={checkCredentials}>
            {"LOG IN TO ACCOUNT"}
          </Button>
          <Button variant="custom" onClick={() => handleRoute("user-type")}>
            {"CREATE AN ACCOUNT"}
          </Button>
          <u className='text-[13px]  sm:text-[18px] text-[#F90] font-bold cursor-pointer' onClick={() => handleRoute("credits")}>{"CREDITS"}</u>
        </div>
      </div>
    </main>
  );
